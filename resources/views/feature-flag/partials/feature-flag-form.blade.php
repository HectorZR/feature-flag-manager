@props(['route', 'method' => 'POST', 'version' => '', 'release_date' => '', 'is_active' => false, 'cancelRoute'])
...
<form action="{{ $route }}" method="post" class="flex flex-col gap-2">
    ...
    <input
        ...
        type="date" id="releaseDate" name="release_date" value="{{ $release_date }}" />
    @error('release_date')
        <x-input-error :messages="$message" />
    @enderror
    ...
    <div class="flex gap-2 justify-start py-2">
        ...
        <input
            ...
            type="checkbox" id="isActive" name="is_active" {{ $is_active ? 'checked' : '' }} value="on" />
    </div>
    @error('is_active')
        <x-input-error :messages="$message" />
    @enderror
